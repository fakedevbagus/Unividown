...
def test_tools_trim_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uploads.settings, "upload_dir", str(tmp_path))
    with TestClient(app) as client:
        response = client.post("/api/tools/trim", files={"file": ("sample.mp4", io.BytesIO(b"media"), "video/mp4")}, data={"start": 1.0, "end": 5.0})
        assert response.status_code == 200
...
def test_upload_filename_is_uuid_and_contained(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uploads.settings, "upload_dir", str(tmp_path))
    with TestClient(app) as client:
        response = client.post("/api/tools/convert", files={"file": ("../../escape.mp4", io.BytesIO(b"media"), "video/mp4")})
    assert response.status_code == 200
...
def test_uploads_with_same_name_do_not_overwrite(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        for content in (b"first", b"second"):
            response = client.post("/api/tools/convert", files={"file": ("same.mp4", io.BytesIO(content), "video/mp4")})
            assert response.status_code == 200
    stored = list(tmp_path.glob("*.mp4"))
    assert len(stored) == 2
...
def test_upload_rejects_unsupported_actual_mime_and_cleans_temp(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uploads.settings, "upload_dir", str(tmp_path))
    with TestClient(app) as client:
        response = client.post("/api/tools/convert", files={"file": ("malware.mp4", io.BytesIO(b"not-media"), "video/mp4")})
...
def test_upload_stream_limit_cleans_partial_file(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        response = client.post("/api/tools/convert", files={"file": ("large.mp4", io.BytesIO(b"12345"), "video/mp4")})
...
def test_processing_job_uses_generated_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uploads.settings, "upload_dir", str(tmp_path))
    with TestClient(app) as client:
        response = client.post("/api/tools/convert", files={"file": ("named.mp4", io.BytesIO(b"media"), "video/mp4")})
    db = SessionLocal()
    try:
